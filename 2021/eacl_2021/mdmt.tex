...
\begin{frame}
\frametitle{Multi domain MT}
\begin{itemize}
\item Training data: $(x_i, y_i, d_i)$
\item IID assumption is violated.
\begin{align*}
D^s &= \sum_{d} \lambda^s_d \mathcal{D}^s_d(x) \\
D^t &= \sum_{d} \lambda^t_{d} \mathcal{D}^t_{d}(x)
\end{align*}
\end{itemize}
\end{frame}
