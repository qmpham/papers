...
A typical setting in machine translation, is to collect the largest possible collection of parallel data for the chosen language pair, with the intent to achieve optimal performance for a task of interest. In such situations, the training data distribution is opportunistic, while the test data distribution is chosen; a key component in training is then to mitigate the detrimental effects of this mismatch between distributions. Single-source and multi-source domain adaptation is a well-studied instance of this setting (see \cite{Chu2017comparison} for a review) , and so is multi-domain learning \cite{Chu18multilingual}\cite{Zeng18multidomain}\cite{Jiang19multidomain}. A related situation is multi-lingual machine translation \cite{Firat16multiway}\cite{Johnson17google}\cite{Ha16towards}\fytodo{Add more recent work}, where the heterogeneity of training data correspond not only to variation in topic or genre, but also in languages.

\fyTodo{Label or covariate shift ?}
This problem is often approached with instance selection or reweighting strategies, where the available training data is used in proportion to its relevance for the test conditions. [Additional techniques : GANs for feau]
...
\section{Formalizing multi-domain translation} \label{sec:mdmt}
...
\IF{$t \equiv 0 \mod{eval\_step}$}
	\STATE{Evaluate current model with K dev sets. Denote $S^t_d$ performance of model at iteration t on domain d.}
...
\cite{Wang20balancing,Wang20learning-multi} formulate the problem optimizing the distribution over the training data in a bi-level optimization problem as follow.
\begin{equation} \label{eq:bilevel}
\begin{split}
D_{trn}^* &= \displaystyle{\mathop{\argmin}_{D_{trn}} J(\theta^*(D_{trn}), D_{dev})} \\
					& \theta^*(D_{trn}) = \displaystyle{\mathop{\argmin} J(\theta, D_{trn})}
\end{split}
\end{equation}
Where
\begin{align*}
J(\theta, D_{dev}) &= \displaystyle{\mathop{\sum}_{x,y \in D_{dev}} l(\theta,x,y)} \\
J(\theta, D_{trn}) &= \displaystyle{\mathop{E_{x,y \sim D_{trn}}[l(\theta,x,y)]}} \\
l(\theta,x,y) &= - log P(\theta,x,y)
\end{align*}
The problem \ref{eq:bilevel} can be approached by coordinate gradient descent alternating between $D_{trn}$ and $\theta$. We limit the search space of $D_{trn}$ to a parameterized differential function $D_{trn}(\psi)$. Given $\psi^t$, $\theta^t$ is update via usual gradient descent as follow
\begin{align*}
\theta^{t+1} &= \theta^t - \mathbf{lr}_{nmt} * \frac{\partial l(\theta^t, x,y)}{\partial \theta} \\
x,y &\sim D_{trn}(\psi^t) 
\\
\end{align*}
Given $\theta^t$, $\psi^t$ is updated via Reinforce algorithm as follow
\begin{align*}
\psi^{t+1} &= \psi^t + \mathbf{lr}_{data} * R(x,y) * \frac{\partial log(D_{trn}(\psi^t)(x,y))}{\partial \psi} \\
x,y &\sim D_{trn}(\psi^t) \\
R(x,y) &= \langle \frac{\partial l(\theta',x,y)}{\partial \theta}, \frac{\partial J(\theta^{t+1},D_{dev})}{\partial \theta} \rangle \\
\theta' &= \theta^t - \mathbf{lr}_{nmt} * \frac{\partial l(\theta^t, x,y)}{\partial \theta} \\
\end{align*}
...
\begin{itemize}
\itemsep0em 
\item Generic models trained with several prefixed mixtures of training data,
\begin{align} \label{mixture:trn}
\lambda^s_d = \frac{q_d^{\alpha}}{\displaystyle{\mathop{\sum}_{i=1}^Kq_i^{\alpha}}} &&
q_i = \frac{\mid D_d \mid}{\displaystyle{\mathop{\sum}_{i=1}^K\mid D_i \mid}}
\end{align} 
with $\alpha \in [0,0.25,0.5,0.75,1.0]$. We denote these systems \system{Mixed-$\alpha$}
