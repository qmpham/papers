...
\subsection{Sampler's parameterization}

\cite{Wang20balancing} used softmax to parameterize the sampler i.e $Q^{trn}(\psi_t) = \text{Softmax}(\psi_t)$ in which $\psi_t \in \mathbb{R}^K$. Parameterizing the sampler by softmax will induce the following gradient.
\begin{align*}
\frac{\partial l}{\partial \psi_i} & = \displaystyle{\mathop{\sum}_{j=1}^K} R_j \frac{\partial P_j}{\partial \psi_i} \\
	...
	& = P_i(R_i-l)\\
\end{align*} 
The fact that the change of $\psi_i$ is proportional to $p_i$ can explain that the optimization often leads to a critical solution in which the sampler ignore almost domains except a few ones. In order to avoid the appearance of the 
