...
To compute the reward $R(d)$ of using data from domain $d$ to train the model, we simulate $k$ training steps from current checkpoint by training the model with $k$ batches sampled from $D^l(d)$ and compute the gain of the weighted dev-loss. This computation is inspired by the target prediction gain in \citep{Graves17automated}. However, \citep{Graves17automated} used accumulated gains from the past as reward, while we predict the usefulness of each domain for improving the future performance of the system given its current state. This is achieved by simulating a round of training with only the data from one domain. Furthermore, the choice of the parameterization of the sampling distribution in our method differs from that of \citet{Graves17automated}, although this choice is empirical.
...
We randomly select in each corpus a development and a test set of 1,000 lines and keep the rest for training. Validation sets are used to chose the best model according to the average BLEU score \citep{Papineni02bleu}.\footnote{We use truecasing and the \texttt{multibleu} script.}\fyDone{A word about meta-parameter settings} Significance testing is performed using bootstrap resampling \cite{Koehn04statistical}, implemented in compare-mt\footnote{\url{https://github.com/neulab/compare-mt}} \citep{Neubig19compare-mt}. We report significant differences at the level of $p=0.05$.\fyDone{Fix correct p value.}\fyTodo{Pourquoi pas sacre BLEU?}
...
In the domain adaptation task, given the data from 6 domains (\domain{med}, \domain{bank}, \domain{law}, \domain{it}, \domain{talk}, \domain{rel}), we aim to build distinct expert NMT models for each domain. In the bi-domain adaptation task, given again same 6 domains, we only focus on adapting only to 2 (known) domains.
...
In the fourth experiment (few shot adaptation)\fyTodo{or weakly supervised ?}, given training data in 6 domains and a small development set of a new domain, (\domain{News} in our case), our goal is to build an NMT model which will perform well on test data from the unseen domain.
...
In this setting, we aim to build an expert NMT model in only one domain. This setting corresponds to Dirac distribution $\vlambda^t$, in which the probability of domains is 0 except for one domain.

We initialize MDAC and DDS by 3 different distributions $\vlambda_0$, $\vlambda_1$ and $\vlambda_d$. According to Table~\ref{tab:da}, MDAC achieves the best performance with $\vlambda_0$ (uniform domain distribution) as the initial value, though $\vlambda_0$  initialization only significantly improves over $\vlambda_d$ initialization in small domains such as \domain{talk},\domain{bank} and\domain{it}. Initializing with $\vlambda_d$ seems more appropriate when targeting large domains such as \domain{med} and \domain{law}. The same conclusions also hold for DDS. 

We now compare the best MDAC system (initialized with $\vlambda_{t=0} = \vlambda_0$) to full finetuning. According to Table~\ref{tab:da}, MDAC is outperformed by fine-tuning method in large domains including \domain{med} and \domain{law}. However, MDAC improves fine-tuning by approximately 0.5 BLEU in \domain{bank} and 1.0 BLEU in \domain{rel}. This indicates that for small domains, out-of-domain data can improve the generalization of the NMT model and MDAC is capable to exploit this advantage by balancing the in-domain and the out-of-domain training data instead of edging out the out-of-domain training data as in fine-tuning.

DDS also demonstrates promising results in this task, but is always outperformed by MDAC overall. The same conclusions also hold for the CL method.\fyTodo{Continue}
...
\begin{table*}[htbp]
  ...
  \begin{tabular}{|l|*8{r|}} \hline
    ...
    FT-Full($d$) &37.7&59.2&54.5&34&46.8&90.8&53.8\\ \hline
    \hline
...
\begin{table*}[htbp]
  ...
  \begin{tabular}{|l|*7{r|}} \hline
    domain \hfill $d=$ & \multicolumn{1}{c|}{\domain{ med}} & \multicolumn{1}{c|}{\domain{ law}} & \multicolumn{1}{c|}{\domain{bank}} & \multicolumn{1}{c|}{\domain{talk}} & \multicolumn{1}{c|}{\domain{ it }} & \multicolumn{1}{c|}{\domain{ rel}} \\ \hline \hline%  \multicolumn{1}{c|}{mean} & \multicolumn{1}{|c|}{\domain{news}} 
    \system{MDAC($\vlambda_0, \vlambda_2$)}&36.2&-&-&-&46.1$^*$&-&\\
    \system{DDS($\vlambda_0, \vlambda_2$)}&36.5&-&-&-&44.4&- \\
    \system{MDAC($\vlambda_0, \vlambda_2$)}&-&57.2$^*$&53.8$^*$&-&-&-& \\
    \system{DDS($\vlambda_0, \vlambda_2$)}&-&56.1&52.6&-&-&-&- \\
    \system{MDAC($\vlambda_0, \vlambda_2$)}&-&-&-&32&-&90 \\
    \system{DDS($\vlambda_0, \vlambda_2$)}&-&-&-&32.9&-&91.1 \\ \hline
  \end{tabular}
...
Most approaches to adaptive / dynamic data selection take inspiration from \citet{Bengio09curriculum}, where the notion of \emph{curriculum learning} (CL) is initially introduced. CL relies on the notion of ``easyness'' of a sample to schedule the presentation of the training data so that the easiest examples presented first and the hardest, last. CL has been shown to speed up learning. While the initial aim was to improve and speed up training, it has also proven useful for domain adaptive / multi-domain / multi-lingual MT, based on alternative definitions of ``easiness''.\fyTodo{Ajouter Grave}
...
\fyTodo{Write conclusion}

Contribution : generic framework for several settings. // comparison of several dynamic strategies.
