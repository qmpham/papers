...
In this setting, we aim to build an expert NMT model in only one domain. This corresponds to setting $\vlambda^t$ to a deterministic distribution, in which the probability of all domains is $0$ except for one domain. We denote this distribution $\vlambda_d$, with $d$ the domain of interest.
...
\begin{table*}[htbp]
  ...
  \begin{tabular}{|l|*7{r|}} \hline
    ...
    \hline
  \end{tabular}
  \caption{Adpating to two domains. For a given line, non empty columns correspond to the pair of target domains. ($^*$) MDAC is significantly better than DDS with $p<0.05$.}
  \label{tab:bi-da}
\end{table*}
