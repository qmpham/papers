...
\label{def:domain}
A domain is a hypothetical probability distribution that generates a set of corpora. We will denote $D(C)$ the domain of the set of corpora $C$.
\end{definition}
...
\center
    ...
    \caption{Sparse word embeddings. The red region contains units that are activated only when an input from the corresponding domain is presented; the other (green) regions are deactivated; the bleu (generic) region is always activated.} 
    \label{fig:network}
  \end{figure}
...
\STATE{Using only generic activate embedding and domain activate embedding for this batch of sentences, i.e, we will have 2 batches of word embedding.\fyTodo{why 2 }}
...
\begin{center}
 ...
 Models & EMEA test & EPPS test & ECB test & khresmoi (EMEA domain) & test 2007 (EPPS domain) & newstest 2009 & newstest 2014 & IWSLT test 2010 \\ [0.5ex] 
 \hline\hline
 Mixed & 64.57 & 26.47 & 68.67 & 21.77 & 23.50 & 13.9 & 10.10 & \\
 \hline
 EMEA-finetuned & & & & & & & & \\
 \hline
 EPPS-finetuned & & & & & & & & \\
