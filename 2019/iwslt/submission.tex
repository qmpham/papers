...
\jcTodo{(explain why conditional GRU are used for the comparison)},
Results are shown by Table~\ref{tab:results-rnn-wdcmt}. 
The comparison is less favourable for our RNN systems, which are arguably much simpler.\footnote{The direct comparison with \texttt{WDCMT} is difficult as the implementation differs in many ways from our RNN: framework, cell types,  etc.}
...
\end{center}
\caption{BLEU scores when comparing \texttt{LDR} to \texttt{WDCMT}.\label{tab:results-rnn-wdcmt}}
\end{table}
