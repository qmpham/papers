...
\jcTodo{highlight the analysis of embeddings as an original technique.}
...
\jcTodo{introduce the additional domain scenario experiment.}
...
\hline
$\mathtt{FT} (EMEA)$ & 76.77$_{49.43}$ & 17.16 & 11.99 &\\
$\mathtt{FT} (EPPS)$  & 20.86 & 37.04 & 24.53 &  \\
$\mathtt{FT} (ECB)$    & 26.93 & 27.09 & 56.52 & \\
\hline
...
\hline
$\mathtt{FT} (EMEA)$ & 68.35$_{42.97}$ & 17.02 & 32.87 & \\
$\mathtt{FT} (EPPS)$  & 36.19 & 26.29 & 40.71 & \\
$\mathtt{FT} (ECB)$   & 24.72 & 18.36 & 74.05 & \\
\hline
...
\hline
$\mathtt{FT} (EMEA)$   & 72.06$_{47.33}$ & 18.62 & 16.78 & \\
$\mathtt{FT} (EPPS)$    & 35.47 $ $ & 34.61 & 39.56 & \\
$\mathtt{FT} (ECB)$      & 21.93 $ $ & 22.60 & 51.53 & \\
\hline
...
\hline
$\mathtt{FT} (EMEA)$  & 65.64$_{44.71}$ & 12.36 & 15.93 & \\
...
\hline
$\mathtt{WDCMT}$           & 68.76 $_{45.29}$ & 35.71 & 52.75 & \\
$\mathtt{LDR_{condgru}}_{pred}$   & 71.70 $_{46.21}$ & 35.09 & 51.22 & \\
\hline
\end{tabular}
} %scalebox
\end{center}
\caption{BLEU scores for RNN systems\label{tab:results-rnn-wdcmt}}
\end{table}
...
We simulate the apparition of the IT domain (see Section~\ref{ssec:data}) once the $\mathtt{LDR}_{oracle}$ model is built for EMEA, EPPS and ECB domains.
Note that to accommodate a new domain the network must reserve the new IT region from the beginning.
...
Then, 100,000 additional iterations are performed using  EMEA, EPPS, ECB and IT data sets ($\mathtt{LDR}_{oracle}^*$).
...
\end{center}
\caption{BLEU scores for the Transformer architecture for varying domain-specific embedding sizes \label{tab:add}}
\end{table}

As expected, a huge gap is observed over the IT test sets when using in-domain data.
Note that $\mathtt{LDR}_{oracle}^*$ clearly outperforms $\mathtt{Mixed}^*$ by a large margin.
As previously, all models show very little differences on the EPPS test set.
Both models show similar results over the ECB test set when further trained with IT data.
Only our $\mathtt{LDR}_{oracle}$ model shows gains on the EMEA test set when further trained (only on the large test set).
Overall better results for our LDR when further trained.
