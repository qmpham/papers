...
We use the author's network implementation\footnote{\noindent\url{http://github.com/DeepLearnXMU/WDCNMT}} that is composed of one bidirectional Gated recurrent units (GRU) layer on the encoder side; and one unidirectional conditional GRU layer on the decoder side; the dimension of ``domain'' layers is~300.
%
Since the comparison is difficult as both networks differ in many ways: framework, cell types, \textit{etc.} we rearrange the architecture of our model to make it closer to that of the \texttt{WDCMT} network. 
This is, we build our model with a single GRU layer in the encoder and a single conditional GRU layer in the decoder ($\mathtt{LDR}_{pred}^{condgru}$).
...
Note that on this test set $\mathtt{LDR}_{oracle}^*$ clearly outperforms $\mathtt{Mixed}^*$ by a large margin.
%
...
Thus, confirming the ability of the model to easily integrate new domains. % as supplementary training data becomes available.
