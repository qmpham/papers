...
The model is composed of 2 Bi-directional LSTM subnetworks, $net_s$ and $net_t$, which respectively encode source and target sentences. Since both $net_s$ and $net_t$ take the same form we describe only the source architecture.
...
In this section we evaluate the performance of our neural similarity classifier and correction algorithm detailed in Section~\ref{sec:similarity}.

\subsection{Divergence Prediction}
...
Table~\ref{results_wemb} shows BLEU~\cite{P02-1040} scores of the same neural MT network when trained over different subsets of the OpenSubtitles data set.
Subsets result from filtering sentences of the original corpus using different similarity thresholds as computed by Equation~\ref{cosine} of our similarity classifier.
As it can be seen, the best performing subset is obtained when using sentence pairs with similarity score in the range $[0.100, +\infty)$. 
This is, training with the most similar $15.5$ million sentences of the original data set. 
...
Note that sentences in the range $[0.076, 0.100]$ ($2.5$ million sentences) does not contribute to increase the accuracy of the resulting neural MT system.
%The BLEU score of the system that considers these sentences in training remains unchanged.
Furthermore, sentences in the range $[0, 0.076]$ (about $6$ million sentences) despite assigned a positive similarity score they degrade the overall performance of the network.

%Note that despite assigned a positive similarity score, sentence pairs in the range $[0, 0.076]$ (aobut $6$ million sentences) does not seem to contribute when training a neural MT network. 
...
\begin{itemize}
\item The entire\footnote{The original Paracrawl corpus contains more than $100$ million sentences. We reduced its size to $22.2$ millions using standard filtering techniques.} data sets (\texttt{all}) of both corpora,
\item The most similar pairs as predicted by our network (\texttt{sim}),
\item After applying the correction algorithm detailed in Section~\ref{correction} (\texttt{sim+fix}).
\end{itemize}
