...
In Appendix \ref{appendix:a}, we prove that $\lim_{\tau \rightarrow 0}\hat{m}_l^d(\tau) = \tilde{m}_l^d$. Furthermore, we also provide the computation of $\hat{m}_l^d(\tau)$ and prove that $\hat{m}_l^d(\tau)$ is a differentiable function w.r.t $\Phi_l^d$ and that its gradients can be computed using the implicit differentiation theorem. During training, we approximate $\tilde{m}_l^d$ by $\hat{m}_l^d(\tau)$ by gradually decaying the hyper-parameter $\tau$ to $0$. The gradient of $D$ w.r.t $\Phi_l^d$ is computed using the chain rule as follows:
\begin{equation}
...
\item Transformer using heuristic multi-task group dropout (\system{HMGD} Transformer): we share 320 nodes for every task, and reserve 32 nodes for each task (making $320 + 32*6 = 512$ nodes in total).
\end{itemize}

We set the dropout probability to 0.1. We train the multi-domain Transformer model for 200k iterations with a batch size of 12k tokens. The convergence of the standard Transformer is before 200K as its validation curve became flat near the 200K-th iteration. The \system{LaMGD} Tranformer converged after 300k iterations with a batch size of 12k tokens. The convergence of \system{LaMGD} is controlled by its validation curve. Finally, we plug domain adapters to the multidomain Transformer model and finetune them for 25k iterations using the same batch size as the baseline. With approximately the same amount of training iterations, \system{LaMGD} and Adapter-based Transformer are trained.
...
\end{align*}
in which $r=0.0001$.
While \citet{Gong21pay,Gong21adaptive} fixed $\tau$ to be 0.2, we combine it with an anneal policy for $\tau$ proposed by previous studies \citep{Jang17categorical} to encourage the diversity in the beginning of the training. Finally, we fix the weight of the entropy $\mathbb{H}(\text{softmax}(\Phi_l^d))$ to $0.0001$ in the training loss in every experiments.
...
We initialize the probabilistic distribution of the latent variables by uniform distribution. More precisely, we set $\Phi_l^d$, which generates the probability of the masks via the softmax activation function, to $0^{n_d}$.
\subsection{Datasets}
...
\begin{table*}[h!]
  ...
  \begin{tabular}{|p{4cm}|*{7}{r|}} \hline
    ...
    \system{Transformer}  \hfill{\footnotesize[65m]} & \SB{40.3} & 59.5 & 49.8 & 36.4 & 49.0 & 80.0  & 52.5\\
    \revision{\system{Adapter}}   \hfill{\footnotesize[+151m]}  & 39.5 & \SB{61.0} & \SB{53.1} & 37.5 & 49.6 & \SB{91.0} & \SB{55.3} \\ 
    \system{LaMGD} Tranformer   \hfill{\footnotesize[+0m]}  & \SB{40.3} & 60.4 & 52.4 & \SB{39.0} & \SB{52.4} & 87.5 & \SB{55.3} \\ 
    \system{HMGD} Tranformer   \hfill{\footnotesize[+0m]}  & \SB{40.4} & 60.4 & 51.9 & 38.7 &	50.80 &	86.80 & 54.8 \\ 
    \hline
  \end{tabular}
...
\begin{table*}[h!]
  ...
  \begin{tabular}{|p{4cm}|*{9}{r|}} \hline
    ...
    \hline
  \end{tabular}
  \caption{Experiments with multilingual translation. Boldface denotes significant gains over \system{Transformer} ($p=0.05$).\fyDone{What is the bold for ?}}
...
\end{subfigure}
\caption{Heatmap visualization of the similarities between dropout masks of domains(languages). Brighter colors correspond to higher similarity scores.}
\label{fig:heatmap}
\end{figure}
...
\begin{table}[h!]
  ...
  \begin{tabular}{|c|c|c|} \hline
    ...
    \hline
  \end{tabular}
  \caption{Variation of the performance w.r.t $k$, while we fix $n_p=16$. These results correspond to the multilingual setting (o2m-related).}
  \label{tab:K}
\end{table}
...
\textbf{Language-specific sub-networks}. \citet{Frankle19Lottery,liu18rethinking} proposed different techniques to locate most important parameters for the task so that when training the model while masking other parameters the performance of the model does not degrade.\citet{lin21learning} hypothesized that among parameters of a multilingual NMT model, there exists a subset of parameters more biased to a specific language. The authors proposed to locate these subsets by pruning the parameters of each fine-tuned model. Our approach also aims to assign a sub-network to each task by masking the output of each layer rather than masking the parameters. Furthermore, \citet{lin21learning} computes the masks via a heuristic selection while our approach learn the masks via a variational technique.
