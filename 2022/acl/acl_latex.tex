...
Finally, we fix the weight of the entropy $\mathbb{H}(\text{softmax}(\Phi_l^d))$ to $0.0001$ in the training loss in every experiments. The choice of the values for our hyper-parameters was not optimized and selected by suggestions from previous studies \citep{Jang17categorical,Gong21pay,Gong21adaptive}.
...
\begin{table}[h!]
  ...
  \begin{tabular}{|c|c|} \hline
    ...
    \hline
  \end{tabular}
  \caption{Variation of the performance w.r.t $k$, while we fix $n_p=16$. These results correspond to the multilingual setting (o2m-diverse).}
  \label{tab:K}
\end{table}
