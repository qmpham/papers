...
\begin{itemize}
\item Transformer: The embedding dimension for both encoder and decoder is set as 512, and the feedforward dimension is 2048; the multi-head attention mechanism contains 8 heads;\fyTodo{How many layers}
...
We set the dropout probability to 0.1. We train the multi-domain Transformer model for 20k iterations with a batch size of 12k tokens. We train \system{LaMGD} Tranformer for 300k iterations with a batch size of 12k tokens: these extra iterations are justified by the need for the \system{LaMGD} Tranformer to learn both the sub-networks and the parameters of the underlying Transformer.\fyTodo{Have we converged?} Finally, we plug domain adapters to the multidomain Transformer model and finetune them for 25k iterations using the same batch size as the baseline.
...
\begin{table*}[h!]
  ...
  \begin{tabular}{|p{4cm}|*{3}{r|}} \hline
    ...
    \system{Adapter}   \hfill{\footnotesize[+25m]}  & 61.0 & 59.4 (-0.6) &  59.2 (-0.8) \\ 
    \system{LaMGD} Transformer   \hfill{\footnotesize[+0m]}  & 61.4 & 61.4 (=) & 61.4 (=) \\ 
    \hline
  \end{tabular}
...
\end{algorithmic}
\end{breakablealgorithm}
\section{Appendix D}\fyTodo{fix this}
\begin{figure*}[h!]
