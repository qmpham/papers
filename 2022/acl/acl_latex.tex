...
\item \system{LaMGD} Transformer:\fyTodo{Explain acronym} There is no change in the architecture. We group the 512 nodes in each layer into 16 groups of 32 consecutive nodes. For each domain, only 12 out of the 16 groups are selected. The number of parameters of the variational distributions is $k \times L \times n_d$, which is negligible when compared to the size of the Transformer model;
\end{itemize}
...
We set the dropout probability as 0.3. We train the multilingual Transformer model in 40~K iterations with a batch size of 9600 tokens on 16 GPUs as in \citet{Gong21adaptive}. We train \system{LaMGD} Tranformer for 50k iterations with the same batch size. Finally, we finetune the language-specific Adapters for 5k iterations.

All the translation systems are implemented OpenNMT\fyTodo{Fix this, add reference to framework if needed}.
...
We use the same data as in the recent work of \citet{Pham21revisiting} on multidomain translation.\fyTodo{Add public to the data link} We describe the datasets for multidomain translation experiments in Table~\ref{tab:Corpora-chap4}.
\begin{table*}[h!]
  \centering
...
The languages used in multilingual experiments are refered to by their ISO 639-1 code that we reproduce below for convenience:
\begin{itemize}
