...
\item We provide a rigorous mathematical formulation to the problem of jointly learning task-dependent sub-networks and the parameters of the underlying models using variational probabilistic modeling techniques;
...
We choose $n_d = 16$ so that the dropout groups' size is not too small or too large. In our future works, we will do more experiments by changing $n_d$ to examine how this hyperparameters affect the performance of the model. The second hyperparameter in \system{LaMGD} is the number of retained groups in each layer, $k$, which we predetermined as 12 in every experiments. By retaining $12/16$ groups, we share $75 \%$ active groups between two domains or languages in average.

Furthermore, the temperature $\tau$ of our $\operatorname{Soft-Top-K}$ operator will be gradually decreased from $0.5$ to $0.2$ as follows
\begin{align*}
...
\subsection{Datasets}
\subsubsection{Multi-domain translation}
We reuse the same data as the recent work of \citet{Pham21revisiting} in multi-domain translation. We describe our datasets for multi-domain translation experiment in Table \ref{tab:Corpora-chap4}
\begin{table*}[h!]
  \centering
...
\begin{table*}[h!]
  ...
  \begin{tabular}{|p{4cm}|*{9}{r|}} \hline
    ...
    \system{Transformer}  \hfill{\footnotesize[96.9m]} & 10.2&4&12.7&22.2&31.8&\SB{34}&38.3&8.3&20.19 \\
    ...
    \system{Transformer}  \hfill{\footnotesize[70.4m]} &22.4&9.7&20.5&31.8&37.5&38.7&39.8&19&27.43 \\
    \revision{\system{Adapter}}   \hfill{\footnotesize[13m]}  &22.5&9.4&20&30.6&37.2&38.2&39.3&19&27.03\\ 
    \system{LaMGD} Tranformer  \hfill{\footnotesize[+0m]} &\SB{23.5}&9.6&\SB{21.5}&32.2&37.7&38.6&40&18.9&27.75 \\
    \hline
  \end{tabular}
...
For multilingual (TED-related) case, the data contain four language families including (1) Turkic family with Azerbaijani and Turkish(\domain{aze},\domain{tur}); (2) Slavic family with Belarusian and Russian (\domain{bel},\domain{rus}); (3) Romance family with Galician and Portuguese (\domain{glg}, \domain{por}); and (4) Czech-Slovak family with Slovak and Czech (\domain{ces}, \domain{slk}). The similarities between the sub-networks for each pair are respectively 0.95, 0.94, 0.99 and 0.96: for each language, the maximally similar one (excluding the diagonoal) is always from the same language family. 
\begin{table*}[h!]
  \centering
...
We also plot the languages based on their dropout masks in Figure~\ref{fig:visual} using a 2d projection computed by Principal Component Analysis (PCA).
\begin{figure*}[htbp]
\begin{subfigure}{0.5\textwidth}
  \centering
  % include first image
  \includegraphics[width=0.85\textwidth]{ted_related}  
  \caption{TED-Related}
