...
For $i^{\text{th}}$layer of the encoder or the decoder, let $h^i$ be the output of the layer, of dimension d. The $i^{\text{th}}$ residual adapter modifies $h^i$ by applying several transformations as follows:\fyTodo{Use align env}
\begin{align*}
  ...
  \bar{h}^{i} &= h^{i}_3 + h^i
\end{align*}
Where matrices $\displaystyle{\mathbf{W}_{db}^{i}\in\mathbb{R}^{d\times b}}$,$\displaystyle{\mathbf{W}_{bd}^{i}\in\mathbb{R}^{b\times d}}$, bias vectors $\displaystyle{b^i_{1} \in \mathbb{R}^{b}}$, $\displaystyle{b^i_{2} \in \mathbb{R}^{d}}$, b is a bottleneck dimension. For the sake of brevity, we denote $h^i_3 = \operatorname{ADAP}^{(i)}(h^i)$.\fyDone{or is it $h_i$ ?}\fyTodo{attention aux matrices $W_i$}

The "adapted" context vector $\bar{h}^i_{ 1\leq i \leq n-1}$, where n is the number of layers, is the input of the $(i+1)^{\text{th}}$ \fyDone{Self attention ?} layer, $\bar{h}^n$ is passed to the decoder if it belongs to the encoder side, or the input of output layer if it belongs to the decoder side. In our architecture, there are 12 residual adapters, one for each of the six attention layers of the encoder and similarly for the decoder \footnote{In the decoder, the stack of self-attention layers and cross encoder-decoder attention only counts as one attention layer and serves only one residual adapter}.

\subsection{Ablation study on position and number of residual adapters \label{ssec:ablatation}}
\fyTodo{Move this}
In this section, we would like to analyze the impact of position and number of residual adapters involved in the adapted model. In fact, the importance of hidden layers varies with respect to their position in the model. It is conjectured that the higher layers represent more global patterns such as semantic while the lower layers represent more local patterns such as syntactic \cite{raganato18analysis}\fyTodo{Missing reference here}. The domain shift in local patterns and global patterns has not yet studied. In this paper, we do not intend to study this aspect of domain adaptation problem. We would like to give an extensive comparison between domain adaptation in different levels in NMT model.\fyTodo{Fix style here} Because the set of possible configurations is large, we only perform experiments on layers at position: 2, 4, 6 in each side of NMT model (encoder and decoder) which covers a wide range in the hierarchy of attention layers. 

Second, we would like to observe how much the number of residual adapters affects the performance of the adapted model. By the limit of computation resource, we limit our study to the case of 3 adapters at positions: 2,4,6 in each side of the NMT model. By doing this, we could cover 3 situations: 1 adapter (mentioned in the previous paragraph), 3 adapters and 6 adapters (in the normal setting) in each side (encoder and decoder).\fyTodo{We have 12 layers, have we not ?}
...
\subsection{Highway connection}
Finally, we would like to assess the performance of a variant of residual adapters in which the adapters go directly to the end of the stack of layers. We graphically demonstrate this version in Figure~\ref{fig:hrl-architecture}. In this version, only the last layers of the encoder and the decoder are modified.
...
The benefit of this variant is that we can manipulate all values of the adapters at once.
...
\begin{table*}[htbp]
  ...
  \begin{tabular}{|l|l|} \hline
    Model & params \\ \hline 
    \system{Transformer}  & 65 \\
   \system{Residual Adapter} & 1 \\
     \hline
  \end{tabular}
  \caption{Number of parameters($\times 10^3$)}
  \label{tab:params}
\end{table*}
