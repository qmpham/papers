...
\end{equation}
where $z_k(h^L(w))$ measures the relatedness of word $w$ to the domain $k$. The more likely $w$ is in domain $k$, the larger $z_k(h^n(w))$ should be; conversely, for words that are not typical of any domain $k$ (eg.\ function words),  adaptation is minimum and the adapted corresponding encoder output will remain close to the output of the generic model ($h^L$). In our implementation, we incorporate two domain classifiers on top of the encoder and the decoder that take the last hidden layer of encoder (resp.\ decoder) as input, and use the posterior probability $P(d=k|w)$ as the value of $z_k(h^n(w))$.

The training of gated residual adapters thus comprises three main steps:
\begin{itemize}
\item Learn a generic model with mixed corpora comprising data from multiple domains.
\item Train a domain classifier on top of the encoder and decoder; during this step, the parameters of the generic model are frozen. This model computes the posterior domain probability $p(k|h(w_t))$ for each word $w_t$ based on the representation computed by the last layer.
