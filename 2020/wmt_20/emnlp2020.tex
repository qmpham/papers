...
\fyTodo{More contexts and notations from the transformer}\fyTodo{Encoder / decoder layers}
For $i^{\text{th}}$layer of the encoder or the decoder, let $h^i$ be the output of the layer, of dimension $d$. The $i^{\text{th}}$ residual adapter modifies $h^i$ by applying several transformations as follows:\fyDone{Use align env}
\begin{align*}
  ...
  \bar{h}^{i} &= h^{i}_3 + h^i.
\end{align*}
Matrices $\displaystyle{\mathbf{W}_{db}^{i}\in\mathbb{R}^{d\times b}}$,$\displaystyle{\mathbf{W}_{bd}^{i}\in\mathbb{R}^{b\times d}}$, bias vectors $\displaystyle{b^i_{1} \in \mathbb{R}^{b}}$, $\displaystyle{b^i_{2} \in \mathbb{R}^{d}}$, b is a bottleneck dimension. For the sake of brevity, we denote $h^i_3 = \operatorname{ADAP}^{(i)}(h^i)$, and $\theta_{\operatorname{ADAP}^{(i)}}$ the set of parameters for the $i^{\text{th}}$ adapters.\fyDone{or is it $h_i$ ?}\fyDone{attention aux matrices $W_i$}

The "adapted" hidden vectors $\bar{h}^i_{ 1\leq i \leq n-1}$, where n is the number of layers, is the input of the $(i+1)^{\text{th}}$ \fyDone{Self attention ?} layer, $\bar{h}^n$ is passed to the decoder if it belongs to the encoder side, or the input of output layer if it belongs to the decoder side. In our architecture, there are 12 residual adapters, one for each of the six attention layers of the encoder and similarly for the decoder.\footnote{In the decoder, the stack of self-attention layers and cross encoder-decoder attention only counts as one attention layer and serves only one residual adapter} 
\fyTodo{Many question opens: how to train ? how many adapters ? which size ? etc}
...
Fine-tuning on small corpora can still lead to an overfitted model even when the fine-tuning is applied to a few residual adapters. To avoid this situation, we have several options including: reducing the size of the adapter module, applying weight decay or layer regularization to the adapter. The first option is not flexible \fyTodo{compatible ???} if we consider the probable growth of in-domain data. The second and third choices are preferable to regularize a large adapter which will scale if the in-domain corpora extend .\fyTodo{Style}

Weight decay applies a penalization on the weights of the adapters.\fyDone{The second summation also runs over $x,y$ ? I think not}
\begin{equation}
  \begin{split}
  ...
  \end{split}
\end{equation}
Where $\theta_{\operatorname{ADAP}^{(i)}}$ are parameters of $i^{\text{th}}$ adapter
...
In a multi-domain context, there are 2 ways to train domain-specific NMT model using adapters including pretraining generic NMT then finetuning adapters; or training both NMT model and adapters at once from the beginning using multi-task learning. In the literature of Machine Learning, \cite{Caruana97multitask} proposed an architecture suitable for multi-task learning. The corresponding network comprises a ground layer, which is shared between tasks and is followed by task-related layers. The network is then trained from scratch by fueling task-related examples in a batch and updating all parameters (shared and task-specific) at the same time. The multi-domain context is similar to the multi-task context if we consider a domain as a task\cite{Dredze08online}. Therefore, we can also use multi-task training to optimize the parameters of NMT model and the parameters of the residual adapters at once from the beginning. To assess the efficacy of multi-task training in the multi-domain context, we would like to compare the finetuning approach with the multi-task training approach. \fyTodo{Change the training regime}

\subsection{Highway Residual Adapters \label{ssec:highway}}
Finally, we would like to assess the performance of a variant of residual adapters in which the adapters go directly to the end of the stack of attention layers. We graphically demonstrate this version in Figure~\ref{fig:hrl-architecture}. In this version, only the last hidden vector of the encoder (decoder) is modified as follow
\begin{equation}
\bar{h}^n = h^n + \displaystyle{\mathop{\sum}_{1 \leq i \leq n} ADAP^i(h^i)}
\end{equation}
The benefit of this variant is that it allows us to reuse hidden vectors $h^i$ of all hidden layers while adapting to multiple domains. In the forward step, the hidden vectors $h^i$ are calculated once, then adapters take these vectors to modify the last layer. Therefore, we can finetune the model to multiple domains at once without recomputing $h^i$. The second minor benefit of this variant is that it allows implementing the Gated Residual Adaptaters version that will be described in the following section \ref{ssec:gate}.
\begin{figure}[htbp]
  \centering
...
\end{figure}
\subsection{Gated Residual Adaptaters \label{ssec:gate}}
\mpTodo{Formalizing problem, network design, training algorithm}
To translate a sentence, we have 2 options: using a domain-adapted model or using the generic model. In the case of choosing the wrong domain, the performance is dramatically worse than using the generic model. Therefore, we would like to design a version of applying residual adapter that allows us to automatically accept or reject given domain information, i.e, even if we choose a wrong adapter, the NMT model is still able to reject the change output from the adapter. In this version, we reuse the highway version of residual adapters. 
\fyTodo{Consistency of notations wrt section 2.1}
Given domain $k$ with set of adapters $ADAP^i_k$, in the model adapted to domain k, instead of using the formulation $\bar{h}^n = h^n + \displaystyle{\mathop{\sum}_{1 \leq i \leq n} ADAP_k^i(h^i)}$ (encoder or decoder's side), we propose "gated" version uses $\bar{h}^n = h^n + \displaystyle{\mathop{\sum}_{1 \leq i \leq n} ADAP_k^i(h^i) \odot{} z_k(h^n)}$. Where $z_k(h^n(w))$ is designed to reflect the relatedness of word $w$ to domain $k$: the more likely $w$ is in domain $k$, the larger $z_k(h^n(w))$ should be. The gated version aims to reduce the change $\displaystyle{\mathop{\sum}_{1 \leq i \leq n} ADAP_k^i(h^i)}$ added to $h^n$ for words that are not typical of domain $k$. In our implementation, we incorporate 2 domain classifiers on top of the encoder and the decoder that take the last hidden layer of encoder (decoder) as input, we take the posterior probability $P(d=k|w)$ as value of $z_k(h^n(w))$.

The training of Gated Residual Adaptaters comprises three main steps:
\begin{itemize}
	\item Pretraining a generic model with mixed corpora comprising data from multiple domains.
...
For all models in En-Fr, we set the embeddings size and the hidden layers size to~512. Transformers use multi-head attention with 8 heads in each of the 6 layers; the inner feedforward layer contains 2048 cells. The multi-domain residual system (see description below) additionally uses an adaptation block in each transformer layer, composed of a 2-layer perceptron, with an inner RELU activation function operating on normalized entries of dimension 1024. 
...
For all models in En-De, we set the embeddings size and the hidden layers size to~1024. Transformers use multi-head attention with 16 heads in each of the 6 layers; the inner feedforward layer contains 4096 cells. The multi-domain residual system (see description below) additionally uses an adaptation block in each transformer layer, composed of a 2-layer perceptron, with an inner RELU activation function operating on normalized entries of dimension 2048. Training uses a batch size of~32768 tokens.

The size of models are reported in the table \ref{tab:params}
...
To measure the robustness against out-of-domain examples, we perform translations with wrong domain information to trick the systems that need to know domain a priori. In the table \ref{tab:performance-random}, the column \domain{RND} stores the score of multi-domain models, that use domain information to translate, in the situation in which we assign random domain tag to each test sentence. 

Beside multi-domain approaches reported in previous section, we report model \system{FT-full-UEW} which is obtained by full-finetuning generic model using uniform elastic weight to avoid catatrophic forgetting.
