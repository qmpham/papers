...
% Residual adapters allow us to adapt the NMT model to any specific domain in a computationally cheap way. Could we adapt the NMT model to noisy text and topical text at once? While residual adapters are good at adapting to one specific domain, their performance still dramatically decreases for the other domains seen in training, as we show in our experiments. Therefore, we have to decide whether to use residual adapters manually, i.e, we have to know to which domain the text belongs a priori. Therefore, we would like to fuse a domain classifier to the architecture in order to weight the contribution of the adapters with respect to the topic-relatedness of the text.
...
In this section, we describe the basic version of the residual adapter architectures \citep{houlsby19parameter, Bapna19simple}, as well as two original variants of this model.
...
The residual adapter architecture extends this model by adding one new computation module for each layer. The $i^{\text{th}}$ residual adapter thus modifies $h^i$ by computing the following transformations:\fyDone{Use align env} \fyTodo{Add layer normalization, replace notation b by a}
\begin{align*}
...
For all En$\rightarrow$Fr models, we set the embeddings size and the hidden layers size to~512. Transformers use multi-head attention with 8 heads in each of the 6 layers; the inner feedforward layer contains 2,048 cells. Residual adapters additionally use an adaptation block in each layer, composed of a 2-layer perceptron, with an inner ReLU activation function operating on normalized entries of dimension $b=1024$.\revision{ \citet{Bapna19simple} showed that the performance of adapted model increases with respect to the size of the inner dimension. In the experiments of \citet{Bapna19simple}, we note that $b=1024$, which is 2-folds larger than the dimension of Transformer layer, achieved performance closed to fine-tuned model. Therefore, we would like to apply this setting to our experiments.}
...
Training uses a batch size of~12,288 tokens; optimization uses Adam with parameters $\beta_1=0.9$, $\beta_2= 0.98$ and Noam decay ($warmup\_steps=4,000$), and a dropout rate of $0.1$ for all layers.\revision{ For \system{mixed} model, we use initial learning 1.0 and the concatenation of the validation sets of 6 domains as the validation set of \system{mixed}. In the fine-tuning experiments, we continue training using \system{mixed} as starting point, we use the same learning rate schedule, continuing the incrementation of the number of steps. In the multi-task training, we use the same learning rate schedule as \system{mixed}, for each iteration, we sample a domain from the pool of 6 domains with the probability proportional to the size of training data of each domain, then we sample a batch of~12,288 tokens from training data of the picked domain, the shared parameters and the parameters of the corresponding adapter are updated by gradients.}\fyDone{Describe the block adaptation layer - voir slides}
...
\begin{table*}[htbp]
  \centering
  \begin{tabular}{|p{4cm}|*{8}{r|}|r|r|} \hline
%     &&&&&& \\
    Model / Domain & \multicolumn{1}{c|}{\domain{ med}} & \multicolumn{1}{c|}{\domain{ law}} & \multicolumn{1}{c|}{\domain{bank}} & \multicolumn{1}{c|}{\domain{talk}} & \multicolumn{1}{c|}{\domain{ it }} & \multicolumn{1}{c|}{\domain{ rel}} & \multicolumn{1}{c||}{\domain{avg}} & \multicolumn{1}{c|}{\domain{rnd}} & \multicolumn{1}{c|}{\domain{news}} & \multicolumn{1}{c|}{\domain{PARAMS}}  \\ \hline % & \multicolumn{1}{c|}{\domain{PROC}} \\ \hline  
    ...
    \system{Res-Adap-HW-MT-ptr} & 37.7 & 57 & 52.5 & 33.5 & 46.1 & 89 & 52.6 & 46.5 & 21.4 & 65M/12M \\ %&  \\
...
%    \system{FT-Full-UEW}      & 37.9 & 56.0 & 52.1 & 33.7 & 44.9 & 89.1 &	52.3 & 46.5 & 22.11 \\ %& 34s \\
    ...
    %\hline
  \end{tabular}
  \caption{Translation performance of highway and gated variants for En$\rightarrow$Fr.\revision{\domain{news} is excluded from training data and considered as out-of-domain test}.
...
Finally, note that a distinct evolution of the residual adapter model of \citet{Bapna19simple} is presented in \citet{Sharaf20metalearning} where meta-learning techniques are used to make fine-tuning more effective in a standard domain-adaptation setting.
